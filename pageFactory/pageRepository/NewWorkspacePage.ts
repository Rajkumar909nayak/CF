...
export class NewWorkspacePage {
   ...
   * Method to enter OTP
   */
  async enterOTP(domain: string, inbox: string, token: string) {
    if (await this.slackRecognizeTitle.isVisible({ timeout: 5000 })) {
      await webActions.fetchMessageId(domain, inbox, token);
      const OTP = await webActions.fetchOTP(domain, inbox, token);
   ...
   */
  async loginToSlack(email: string, domain: string, inbox: string, token: string) {
    ...
    await this.enterOTP(domain, inbox, token);
  ...
  async navigateToSlackWorkspace() {
    if (await this.clickOnSlackLaunch.isVisible({ timeout: 15000 })) {
  ...
  async navigateSlackWorkspace() {
    await this.page.waitForTimeout(3000);
    await this.againNavigateToSlack();
  }

  async verifyTikcet1(newPage: Page) {
    await this.page
      .locator('(//span[contains(text(),"test12")])[1]')
  ...
  async verifyCommentsFromWebAppToSlack1() {
