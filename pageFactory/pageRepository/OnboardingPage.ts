...
export class OnboardingPage {
   ...
   * @param password
   */
  async loginWithMagicLink(onBoardingPage: Page, email: string, domain: string, inbox: string, token: string) {
    ...
    await magicLinkPage.goto(await webActions.fetchMagicLink(domain, inbox, token));
   ...
   * @param cfpage
   */
  async loginToClearFeedWithMagicLink(cfpage: Page, email: string, domain: string, inbox: string, token: string) {
    await this.loginWithMagicLink(cfpage, email, domain, inbox, token);
  ...
  async verifyCollections(onBoardingPage: Page) {
    await onBoardingPage
      .locator('//*[contains(text(),"COLLECTIONS")]')
      .isVisible();
    await onBoardingPage
  ...
  async verifySettingConfiguration(onBoardingPage: Page, index: number) {
    // await page2
    //   .locator('//*[contains(text(),"COLLECTIONS")]')
    //   .isVisible();
    // await page2
    ...
    if (await onBoardingPage.locator('//span[@class="ant-switch-inner-checked"]').isChecked()) {
      ...
      // await page2.locator('//span[@class="ant-switch-inner"]').click();
    }
    else if (!await onBoardingPage.locator('//span[@class="ant-switch-inner-unchecked"]').isChecked()) {
      await onBoardingPage.locator('//span[@class="ant-switch-inner"]').click();
    }
    await onBoardingPage.locator('//div[@class="ant-select-selector"]').click();
    ...
    await onBoardingPage.locator('(//input[@class="ant-radio-input"])[3]').click();
    await onBoardingPage.locator('//button[@type="submit"]').click();
    await onBoardingPage.locator('//div[@title="Requests"]').click();
  }

  async verifyCommentsFromSlckToWebApp(onBoardingPage: Page) {
    // await page2.locator('(//div[@class="ant-table-tbody-virtual-holder-inner"]//div)[1]').click();
    ...
    await onBoardingPage.locator('//button[contains(@class,"ant-btn ant-btn-default px-2 story_drawerCloseButton__cEe_Q")]').click();
