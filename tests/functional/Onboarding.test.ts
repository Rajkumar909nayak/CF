  ...
  test.skip(
    ...
    async ({ newWorkspacePage, onboardingPage }) => {
      ...
      await test.step('Navigate to existing Slack workspace', async () => {
        await newWorkspacePage.loginToSlack(testData.userEmail, testData.domainName, testData.inboxName, testData.tokenKey);
      });
      await test.step('Navigate to ClearFeed application Using Magic link', async () => {
        ...
        await onboardingPage.loginToClearFeedWithMagicLink(CFPage, testData.userEmail, testData.domainName, testData.inboxName, testData.tokenKey);
